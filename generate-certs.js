...
<head>
  ...
  <title>Certificate of ${data.certificate_type} - ${data.student_name}</title>
  ...
  <style>
    ...
    * {
    ...
    .logo-iso {
      background-position: -62.325px 0px;
      width: 62.325px;
      height: 64px;
    }

    .logo-arms {
      background-position: -124.65px 0px;
      width: 62.325px;
      height: 64px;
    }

    .logo-uk {
      background-position: -186.975px 0px;
      width: 62.325px;
      height: 64px;
    }
...
<body>
  ...
  <div class="cert-container">
    ...
    <!-- Main Content Area -->
    <div class="content">
      <h1 class="title">CERTIFICATE OF ${data.certificate_type}</h1>
...
async function main() {
  ...
  try {
    ...
    console.log(`Found ${res.rows.length} certificate records in database.`);
    ...
    for (const row of res.rows) {
      ...
      const filepath = path.join('c:\\Users\\USER\\OneDrive\\Desktop\\adhira', filename);

      console.log(`Generating ${filename} for ${row.student_name}...`);
      const htmlContent = certTemplate(formattedData);
      fs.writeFileSync(filepath, htmlContent, 'utf8');
      console.log(`Saved ${filename} to disk.`);
    }

    console.log('All local static certificate HTML files successfully updated!');
  } catch (err) {
